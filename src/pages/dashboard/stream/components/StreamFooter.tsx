...
export function StreamFooter({
  ...
  return (
    <div className="pointer-events-none absolute inset-x-0 bottom-0 z-20 bg-linear-to-t from-black via-black/60 to-transparent px-2.5 pb-2.5 pt-12 sm:px-5 sm:pb-5 sm:pt-20 lg:px-8">
      <div className="flex h-full flex-col justify-end gap-2.5 sm:gap-4">
        {isInvitedPending && (
          <div
        ...
        <div className="pointer-events-auto w-full max-w-[min(100%,25rem)] sm:max-w-sm">
        ...
        <form
          ...
          className="pointer-events-auto flex items-center gap-2 rounded-[1.2rem] border border-white/10 bg-black/35 p-1.5 backdrop-blur-xl sm:gap-3 sm:rounded-[1.5rem] sm:p-2"
          onClick={(event) => event.stopPropagation()}
        >
          <div className="flex-1 rounded-[1.1rem] border border-white/10 bg-white/10 backdrop-blur-md">
            <input
              ...
              placeholder="Comment..."
              className="w-full bg-transparent px-3.5 py-2.5 text-[10px] text-white placeholder:text-zinc-400 focus:outline-none sm:px-5 sm:py-3 sm:text-xs"
            />
          </div>
          <button
            type="submit"
            disabled={isSubmitting}
            className="flex h-10 w-10 shrink-0 items-center justify-center rounded-[0.9rem] bg-brand-primary text-white transition-colors hover:bg-brand-accent hover:text-black disabled:opacity-50 sm:h-12 sm:w-12 sm:rounded-[1rem]"
          >
            <Send size={16} />
          </button>
        </form>
